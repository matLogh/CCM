...
double reference_time_bgn = 34990;
double reference_time_end = 35050;

std::shared_ptr<TH2F> get_matrix()
{
    TFile                *f = TFile::Open(std::string("data/sum_temat_137Cs_Run_27.root").c_str());
    std::shared_ptr<TH2F> mat((TH2F *)f->Get("TEmat_0"));
...
int main(int argc, char **argv)
{
    ...
    ROIs.emplace_back(RegionOfInterest(TEMAT, 11500., 11640., -40., 100., 11560.));
    ...
    c0.Divide(1, 2);
    c0.cd(1);
    TEMAT->GetYaxis()->SetRangeUser(11000, 12000);
    TEMAT->SetTitle("Original matrix");
    TEMAT->Draw("COLZ");
    c0.cd(2);
    TEMAT_fixed->GetYaxis()->SetRangeUser(11000, 12000);
    ...
    std::cout << "TOTAL DURATION OF " << static_cast<double>(duration) / (double)1E6 << " seconds" << std::endl;
