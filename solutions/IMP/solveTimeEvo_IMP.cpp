...
std::string get_imp_matrix_name()
{
    if (!gMATRIX_NAME.empty()) { return gMATRIX_NAME; }
    return normalize_imp_group(gDETECTOR_GROUP) + std::to_string(gDETECTOR);
}

std::filesystem::path get_imp_output_base()
{
    const std::filesystem::path root_path(gROOTFILE);
    auto                        parent = root_path.parent_path();
    if (parent.empty()) { parent = "."; }

    return parent / ("HistAll_" + get_imp_matrix_name());
}

std::string get_output_conffilename() { return (get_imp_output_base().string() + "_TimeEvoCC.conf"); }

std::string get_output_diagnostic_filename() { return (get_imp_output_base().string() + "_correctedTimeEvo.root"); }

std::string get_output_minimization_filename() { return (get_imp_output_base().string() + "_CCMconf.txt"); }
...
std::shared_ptr<TH2> get_imp_matrix_from_file(TFile *matfile)
{
    if (!matfile || matfile->IsZombie()) { throw std::runtime_error("Error! could not open/find the " + gROOTFILE + " file"); }

    const std::string matrix_name = get_imp_matrix_name();
    std::shared_ptr<TH2> TEMAT((TH2 *)matfile->Get(matrix_name.c_str()));
    if (!TEMAT) { throw std::runtime_error("Error! could not open/find the " + matrix_name + " matrix"); }
...
{
    std::cout << "Running final corrections for IMP matrix " << get_imp_matrix_name() << " with super settings..."
              << std::endl;
    ...
    if (!gREFERENCE_VECTORS.empty())
    {
        ...
        {
            throw std::runtime_error("Number of shared reference vectors does not match number of ROIs in solveTimeEvo_IMP");
        }
...
void set_reference_vector()
{
    ...
    auto   TEMAT   = get_imp_matrix_from_file(matfile);
...
void run_chained_ranges(const ccm_settings &)
{
    return;
}
...
void print_help()
{
    std::cout << "Usage: solveTimeEvo_IMP [OPTIONS]\n\n"
              << "Options:\n";
    ...
    std::cout << "  --histogram <1>            Explicit TH2D name, e.g. hetg0 or hetl25; overrides --group/--detector lookup\n";
...
void parse_args(int argc, char **argv)
{
    for (int i = 1; i < argc; ++i)
    {
        ...
        else if (arg == "--group" || arg == "--detector_group")
        {
            ...
            {
                throw std::runtime_error("Missing value for --group");
            }
        }
        else if (arg == "--histogram" || arg == "--matrix")
        {
            if (i + 1 < argc) { gMATRIX_NAME = argv[++i]; }
            else
            {
                throw std::runtime_error("Missing value for --histogram");
            }
        }
        ...
        else if (arg == "--start_run" || arg == "--start" || arg == "--end_run" || arg == "--end" || arg == "--chain_ranges")
        {
            throw std::runtime_error(arg + " is not used by IMP HistAll.root files; select flat histograms with --group/--detector or --histogram");
        }
    ...
    if (gMATRIX_NAME.empty() && gDETECTOR == -1)
    {
        print_help();
        throw std::runtime_error("--detector is required unless --histogram is provided");
    }
    if (gMATRIX_NAME.empty() && gDETECTOR_GROUP.empty())
    {
        print_help();
        throw std::runtime_error("--group is required unless --histogram is provided");
    }
    if (gROIarrs.empty())
    {
        print_help();
    ...
    gMATRIX_NAME = get_imp_matrix_name();

    // Print all parsed input parameters
    std::cout << "Parsed Input Parameters:" << std::endl;
    std::cout << "  Root File: " << gROOTFILE << std::endl;
    std::cout << "  Detector Group: " << (gDETECTOR_GROUP.empty() ? "(explicit histogram)" : gDETECTOR_GROUP) << std::endl;
...
int main(int argc, char **argv)
{
    ...
    TFile *matfile = TFile::Open(gROOTFILE.c_str(), "READ");
    auto   TEMAT_original = get_imp_matrix_from_file(matfile);
