...
{
    gCurrentDet = det;
    TString group_name(group);
    group_name.ToLower();
    if (group_name == "g") { group_name = "hetg"; }
    if (group_name == "l") { group_name = "hetl"; }
    if (group_name != "hetg" && group_name != "hetl") {
        std::cerr << "Invalid IMP group: " << group << ". Expected g, l, hetg, or hetl." << std::endl;
        return;
    }
    gCurrentGroup = group_name;
    TString hist_name = Form("%s%d", group_name.Data(), det);

    TString file_name = Form("HistAll_%s_correctedTimeEvo.root", hist_name.Data());

    TString top_name = hist_name;
    TString fixed_name = Form("%s_fixed", hist_name.Data());

    TString top_label = "raw";
    TString fixed_label = "fixed";
    ...
    gCanvas_timeevo =
        new TCanvas(Form("c_%s", hist_name.Data()),
                    Form("%s and %s %s",
                         top_label.Data(),
                         fixed_label.Data(),
                         hist_name.Data()),
    ...
    std::cout << "Loaded IMP matrix " << hist_name << std::endl;
    std::cout << top_label << ": " << top_name << std::endl;
    std::cout << fixed_label << ": " << fixed_name << std::endl;
}

void LoadDetWidth(int det, const char* group, int canvas_width)
{
    LoadDet(det, group, false, canvas_width);
}
